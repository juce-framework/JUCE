...
public:
    ...
    */
    ...
    /** Initializes a PositionedRectangle from a saved string version.
    ...
    */
    ...
    /** Returns a string version of this position, from which it can later be
        ...
        - If the X or Y coordinate is relative to the parent's center, the number has "C"
          appended to it, e.g. "-50C" would be 50 pixels left of the parent's center.
        ...
        - If the X or Y coordinate should be anchored at the component's center, then it
          has "c" appended to it. So "-50Rc" would mean that this component's
          center should be 50 pixels left of the parent's right-hand edge. "40%c" means that
          this component's center should be placed 40% across the parent's width.
    ...
    */
    ...
    enum AnchorPoint
    {
        ...
        anchorAtCenter                 = 1 << 2     /**< The x or y coordinate specifies where the center of the rectangle should be. */
    };
    ...
    enum PositionMode
    {
        ...
        absoluteFromParentCenter        = 1 << 5,   /**< The x or y coordinate specifies an absolute distance from the parent's center. */
    ...
    */
    ...
    {
        return (AnchorPoint) (xMode & (anchorAtLeftOrTop | anchorAtRightOrBottom | anchorAtCenter));
    }
    ...
    */
    PositionMode getPositionModeX() const noexcept
    {
        return (PositionMode) (xMode & (absoluteFromParentTopLeft | absoluteFromParentBottomRight
                                         | absoluteFromParentCenter | proportionOfParentSize));
    }
    ...
    */
    ...
    {
        return (AnchorPoint) (yMode & (anchorAtLeftOrTop | anchorAtRightOrBottom | anchorAtCenter));
    }
...
private:
    ...
    void addPosDescription (String& s, const uint8 mode, const double value) const noexcept
    {
        ...
        else
        {
            ...
            if ((mode & absoluteFromParentBottomRight) != 0)
                s << 'R';
            else if ((mode & absoluteFromParentCenter) != 0)
                s << 'C';
        }

        if ((mode & anchorAtRightOrBottom) != 0)
            s << 'r';
        else if ((mode & anchorAtCenter) != 0)
            s << 'c';
    }
                          ...
                          const int parentPos, const int parentSize) const noexcept
    {
        ...
        else if ((xMode_ & absoluteFromParentCenter) != 0)
        ...
        if ((xMode_ & anchorAtRightOrBottom) != 0)
            xOut -= wOut;
        else if ((xMode_ & anchorAtCenter) != 0)
            xOut -= wOut / 2;
    }
