...
void LibraryModule::addLibsToExporter(ProjectExporter& exporter) const
{
	...
	if (exporter.isXcode())
	{
		...
		auto embeddedFrameworks = moduleInfo[xcodeExporter.isOSX() ? "EmbeddedOSXFrameworks" : "EmbeddediOSFrameworks"].toString();
		StringArray relativePathEmbeddedFrameworks;
		relativePathEmbeddedFrameworks.addTokens(embeddedFrameworks, ", ", {});
		
		for (auto& framework : relativePathEmbeddedFrameworks)
		{
			File frameworkFile = getFolder().getChildFile(framework);
			File xcodeExporterFile = xcodeExporter.getTargetFolder();
			build_tools::RelativePath relativePath(frameworkFile, xcodeExporterFile, build_tools::RelativePath::buildTargetFolder);
			xcodeExporter.xcodeEmbeddedFrameworks.add(relativePath.toUnixStyle());
		}
