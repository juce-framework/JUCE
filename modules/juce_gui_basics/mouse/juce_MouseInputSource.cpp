...
public:
    ...
    void handleUnboundedDrag (Component& current)
    {
        ...
        {
            auto componentCenter = current.getScreenBounds().toFloat().getCenter();
            unboundedMouseOffset += (lastScreenPos - ScalingHelpers::scaledScreenPosToUnscaled (componentCenter));
            setScreenPosition (componentCenter);
        }
