  ...
  void append(UniformTextSection &other) {
    if (!other.atoms.isEmpty()) {
      ...
      if (!atoms.isEmpty()) {
        ...
        if (!CharacterFunctions::isWhitespace(
                lastAtom.atomText.getLastCharacter())) {
          ...
          if (!CharacterFunctions::isWhitespace(first.atomText[0])) {
            ...
            lastAtom.width =
                font.getStringWidthFloat(lastAtom.getText(passwordChar));
            ++i;
          }
        }
      }
  ...
  UniformTextSection *split(int indexToBreakAt) {
    ...
    for (int i = 0; i < atoms.size(); ++i) {
      ...
      if (indexToBreakAt >= index && indexToBreakAt < nextIndex) {
        ...
        secondAtom.width =
            font.getStringWidthFloat(secondAtom.getText(passwordChar));
...
private:
  void initialiseAtoms(const String &textToParse) {
    ...
    while (!text.isEmpty()) {
      ...
      atom.width = (atom.isNewLine()
                        ? 0.0f
                        : font.getStringWidthFloat(atom.getText(passwordChar)));
...
struct TextEditor::Iterator {
  Iterator(const TextEditor &ed)
      : sections(ed.sections), justification(ed.justification),
        bottomRight((float)ed.getMaximumTextWidth(),
                    (float)ed.getMaximumTextHeight()),
        wordWrapWidth((float)ed.getWordWrapWidth()),
        passwordCharacter(ed.passwordCharacter), lineSpacing(ed.lineSpacing),
  ...
  bool next() {
    ...
    if (atomIndex >= currentSection->atoms.size() - 1) {
      if (atomIndex >= currentSection->atoms.size()) {
        ...
        if (!lastAtom.isWhitespace()) {
          // handle the case where the last atom in a section is actually part
          // of the same word as the first atom of the next section...
          float right = atomRight + lastAtom.width;
          float lineHeight2 = lineHeight;
          float maxDescent2 = maxDescent;

          for (int section = sectionIndex + 1; section < sections.size();
               ++section) {
    ...
    if (shouldWrap(atomRight) || forceNewLine) {
      if (atom->isWhitespace()) {
        // leave whitespace at the end of a line, but truncate it to avoid
        // scrolling
        atomRight = jmin(atomRight, wordWrapWidth);
      } else if (shouldWrap(atom->width)) // atom too big to fit on a line, so
                                          // break it up..
      {
        longAtom = *atom;
        longAtom.numChars = 0;
  ...
  float getJustificationOffsetX(float lineWidth) const {
    if (justification.testFlags(Justification::horizontallyCentred))
      return jmax(0.0f, (bottomRight.x - lineWidth) * 0.5f);
    if (justification.testFlags(Justification::right))
      return jmax(0.0f, bottomRight.x - lineWidth);

    return 0;
  }

  //==============================================================================
  void draw(Graphics &g, const UniformTextSection *&lastSection,
            AffineTransform transform) const {
    if (atom == nullptr)
      return;

    if (passwordCharacter != 0 ||
        (underlineWhitespace || !atom->isWhitespace())) {
      ...
      ga.addLineOfText(currentSection->font,
                       atom->getTrimmedText(passwordCharacter), atomX,
                       (float)roundToInt(lineY + lineHeight - maxDescent));
      ga.draw(g, transform);
    }
  }

  void drawUnderline(Graphics &g, Range<int> underline, Colour colour,
                     AffineTransform transform) const {
    auto startX = roundToInt(indexToX(underline.getStart()));
    auto endX = roundToInt(indexToX(underline.getEnd()));
    auto baselineY =
        roundToInt(lineY + currentSection->font.getAscent() + 0.5f);

    Graphics::ScopedSaveState state(g);
    g.addTransform(transform);
    g.reduceClipRegion({startX, baselineY, endX - startX, 1});
    g.fillCheckerBoard({(float)endX, (float)baselineY + 1.0f}, 3.0f, 1.0f,
                       colour, Colours::transparentBlack);
  }

  void drawSelectedText(Graphics &g, Range<int> selected,
  ...
  float indexToX(int indexToFind) const {
    ...
    GlyphArrangement g;
    g.addLineOfText(currentSection->font, atom->getText(passwordCharacter),
                    atomX, 0.0f);
  ...
  //==============================================================================
  bool getCharPosition(int index, Point<float> &anchor,
  ...
  Rectangle<int> getTextBounds(Range<int> range) const {
    ...
    return Rectangle<float>(startX, lineY, endX - startX,
...
struct TextEditor::InsertAction : public UndoableAction {
  InsertAction(TextEditor &ed, const String &newText, int insertPos,
               const Font &newFont, Colour newColour, int oldCaret,
               int newCaret)
      : owner(ed), text(newText), insertIndex(insertPos), oldCaretPos(oldCaret),
        newCaretPos(newCaret), font(newFont), colour(newColour) {}
  ...
  bool undo() override {
    owner.remove({insertIndex, insertIndex + text.length()}, nullptr,
                 oldCaretPos);
    return true;
  }

  int getSizeInUnits() override { return text.length() + 16; }
...
struct TextEditor::RemoveAction : public UndoableAction {
  RemoveAction(TextEditor &ed, Range<int> rangeToRemove, int oldCaret,
               int newCaret, const Array<UniformTextSection *> &oldSections)
      : owner(ed), range(rangeToRemove), oldCaretPos(oldCaret),
...
struct TextEditor::TextHolderComponent : public Component,
  ...
  ~TextHolderComponent() override { owner.getTextValue().removeListener(this); }

  void paint(Graphics &g) override { owner.drawContent(g); }

  void restartTimer() { startTimer(350); }

  void timerCallback() override { owner.timerCallbackInt(); }

  void valueChanged(Value &) override { owner.textWasChangedByValue(); }
...
struct TextEditor::TextEditorViewport : public Viewport {
  ...
  void visibleAreaChanged(const Rectangle<int> &) override {
    if (!reentrant) // it's rare, but possible to get into a feedback loop as
                    // the viewport's scrollbars appear and disappear, causing
                    // the wrap width to change.
    {
...
TextEditor::TextEditor(const String &name, juce_wchar passwordChar)
    : Component(name), passwordCharacter(passwordChar) {
...
bool TextEditor::undoOrRedo(const bool shouldUndo) {
  if (!isReadOnly()) {
    ...
    if (shouldUndo ? undoManager.undo() : undoManager.redo()) {
      repaint();
      textChanged();
...
bool TextEditor::isMultiLine() const { return multiline; }
...
bool TextEditor::isTextInputActive() const {
  return !isReadOnly() && (!clicksOutsideDismissVirtualKeyboard ||
                           globalMouseListener.lastMouseDownInEditor());
}
...
void TextEditor::setPopupMenuEnabled(bool b) { popupMenuEnabled = b; }
...
void TextEditor::parentHierarchyChanged() { lookAndFeelChanged(); }
...
void TextEditor::updateCaretPosition() {
  if (caret != nullptr && getWidth() > 0 && getHeight() > 0) {
    Iterator i(*this);
    ...
    if (auto *handler = getAccessibilityHandler())
      handler->notifyAccessibilityEvent(
          AccessibilityEvent::textSelectionChanged);
  }
}

TextEditor::LengthAndCharacterRestriction::LengthAndCharacterRestriction(
    int maxLen, const String &chars)
    : allowedCharacters(chars), maxLength(maxLen) {}

String TextEditor::LengthAndCharacterRestriction::filterNewText(
    TextEditor &ed, const String &newInput) {
  String t(newInput);
  ...
  if (maxLength > 0)
    t = t.substring(0, maxLength - (ed.getTotalNumChars() -
...
void TextEditor::setText(const String &newText, bool sendTextChangeMessage) {
  ...
  if (newLength != getTotalNumChars() || getText() != newText) {
    ...
    clearInternal(nullptr);
    insert(newText, 0, currentFont, findColour(textColourId), nullptr,
           caretPosition);
...
void TextEditor::checkFocus() {
  if (!wasFocused && hasKeyboardFocus(false) &&
...
void TextEditor::repaintText(Range<int> range) {
  if (!range.isEmpty()) {
    ...
    textHolder->repaint(0, roundToInt(y1 + offset), textHolder->getWidth(),
                        roundToInt((float)y2 - y1 + offset));
  }
}
...
int TextEditor::getCaretPosition() const { return caretPosition; }
...

{
  ...
  else if (desiredCaretX >
           jmax(0, viewport->getMaximumVisibleWidth() - (wordWrap ? 2 : 10)))
    vx += desiredCaretX + (isMultiLine() ? proportionOfWidth(0.2f) : 10) -
          viewport->getMaximumVisibleWidth();

  vx = jlimit(
      0,
      jmax(0, textHolder->getWidth() + 8 - viewport->getMaximumVisibleWidth()),
      vx);
  ...
  if (!isMultiLine()) {
    ...
    vy = jlimit(
        0,
        jmax(0, textHolder->getHeight() - viewport->getMaximumVisibleHeight()),
        vy);
    ...
    if (desiredCaretY < 0)
      vy = jmax(0, desiredCaretY + vy);
                                         ...
                                         caretRect.getHeight()))
      vy += desiredCaretY + 2 + caretRect.getHeight() -
...
Rectangle<int> TextEditor::getCaretRectangleForCharIndex(int index) const {
  Point<float> anchor;
  auto cursorHeight =
      currentFont.getHeight(); // (in case the text is empty and the call below
                               // doesn't set this value)
  getCharPosition(index, anchor, cursorHeight);

  return Rectangle<float>{anchor.x, anchor.y, 2.0f, cursorHeight}
             .getSmallestIntegerContainer() +
         getTextOffset();
}
...
Point<int> TextEditor::getTextOffset() const noexcept {
  ...
  return {
      getLeftIndent() + borderSize.getLeft() - viewport->getViewPositionX(),
      roundToInt((float)getTopIndent() + (float)borderSize.getTop() + yOffset) -
...
RectangleList<int> TextEditor::getTextBounds(Range<int> textRange) const {
  ...
  while (i.next()) {
    if (textRange.intersects(
            {i.indexInText, i.indexInText + i.atom->numChars})) {
...
int TextEditor::getWordWrapWidth() const {
  return wordWrap ? getMaximumTextWidth() : std::numeric_limits<int>::max();
}

int TextEditor::getMaximumTextWidth() const {
  return jmax(1,
              viewport->getMaximumVisibleWidth() - leftIndent - rightEdgeSpace);
}
...
void TextEditor::checkLayout() {
  if (getWordWrapWidth() > 0) {
    ...
    viewport->setScrollBarsShown(
        ...
        scrollbarVisible && multiline && !wordWrap &&
...
BorderSize<int> TextEditor::getBorder() const { return borderSize; }
...
void TextEditor::scrollToMakeSureCursorIsVisible() {
  ...
  if (keepCaretOnScreen) {
    ...
    auto caretRect =
        getCaretRectangle().translated(leftIndent, topIndent) - getTextOffset();
    ...
    } else if (relativeCursor.x > jmax(0, viewport->getMaximumVisibleWidth() -
                                              (wordWrap ? 2 : 10))) {
      viewPos.x += relativeCursor.x +
                   (isMultiLine() ? proportionOfWidth(0.2f) : 10) -
                   viewport->getMaximumVisibleWidth();
    }

    viewPos.x = jlimit(0,
                       jmax(0, textHolder->getWidth() + 8 -
                                   viewport->getMaximumVisibleWidth()),
                       viewPos.x);
    ...
    } else if (relativeCursor.y > jmax(0, viewport->getMaximumVisibleHeight() -
                                              caretRect.getHeight())) {
      viewPos.y += relativeCursor.y + 2 + caretRect.getHeight() -
                   viewport->getMaximumVisibleHeight();
    }
...
void TextEditor::moveCaretTo(const int newPosition, const bool isSelecting) {
  if (isSelecting) {
    ...
    if (dragType == notDragging) {
      if (std::abs(getCaretPosition() - selection.getStart()) <
          std::abs(getCaretPosition() - selection.getEnd()))
    ...
    if (dragType == draggingSelectionStart) {
      ...
      setSelection(
          Range<int>::between(getCaretPosition(), selection.getStart()));
    }
...
int TextEditor::getTextIndexAt(const int x, const int y) const {
  ...
  return indexAtPosition((float)(x - offset.x), (float)(y - offset.y));
}
...
int TextEditor::getCharIndexForPoint(const Point<int> point) const {
  return getTextIndexAt(isMultiLine() ? point
                                      : getTextBounds({0, getTotalNumChars()})
                                            .getBounds()
...
void TextEditor::insertTextAtCaret(const String &t) {
  const auto filtered =
      inputFilter != nullptr ? inputFilter->filterNewText(*this, t) : t;
...
void TextEditor::setHighlightedRegion(const Range<int> &newSelection) {
  ...
  const auto cursorAtStart =
      newSelection.getEnd() == getHighlightedRegion().getStart() ||
      newSelection.getEnd() == getHighlightedRegion().getEnd();
  moveCaretTo(cursorAtStart ? newSelection.getEnd() : newSelection.getStart(),
              false);
  moveCaretTo(cursorAtStart ? newSelection.getStart() : newSelection.getEnd(),
              true);
}
    ...
    for (auto &underlinedSection : underlinedSections) {
      ...
      while (i2.next() && i2.lineY < (float)clip.getBottom()) {
        if (i2.lineY + i2.lineHeight >= (float)clip.getY() &&
            underlinedSection.intersects(
                {i2.indexInText, i2.indexInText + i2.atom->numChars})) {
...
void TextEditor::paintOverChildren(Graphics &g) {
  if (textToShowWhenEmpty.isNotEmpty() && (!hasKeyboardFocus(false)) &&
      getTotalNumChars() == 0) {
    g.setColour(colourForTextWhenEmpty);
    g.setFont(getFont());

    Rectangle<int> textBounds(leftIndent, topIndent,
...
void TextEditor::addPopupMenuItems(PopupMenu &m, const MouseEvent *) {
  ...
  if (passwordCharacter == 0) {
    ...
    m.addItem(StandardApplicationCommandIDs::copy, TRANS("Copy"),
              !selection.isEmpty());
  }
  ...
  if (getUndoManager() != nullptr) {
    m.addItem(StandardApplicationCommandIDs::undo, TRANS("Undo"),
              undoManager.canUndo());
    m.addItem(StandardApplicationCommandIDs::redo, TRANS("Redo"),
              undoManager.canRedo());
  }
}
...
void TextEditor::mouseDown(const MouseEvent &e) {
  ...
  if (wasFocused || !selectAllTextWhenFocused) {
    if (!(popupMenuEnabled && e.mods.isPopupMenu())) {
      ...
      m.showMenuAsync(PopupMenu::Options(), [safeThis = SafePointer<TextEditor>{
                                                 this}](int menuResult) {
...
void TextEditor::mouseDoubleClick(const MouseEvent &e) {
  ...
  if (e.getNumberOfClicks() > 3) {
    ...
    while (tokenEnd < totalLength) {
      ...
      // (note the slight bodge here - it's because iswalnum only checks for
      // alphabetic chars in the current locale)
...
//==============================================================================
bool TextEditor::moveCaretWithTransaction(const int newPos,
...
bool TextEditor::moveCaretUp(bool selecting) {
  ...
  return moveCaretWithTransaction(indexAtPosition(caretPos.getX(), newY),
                                  selecting);
}
...
bool TextEditor::moveCaretDown(bool selecting) {
  ...
  const auto caretPos = (getCaretRectangle() - getTextOffset()).toFloat();
  return moveCaretWithTransaction(
      indexAtPosition(caretPos.getX(), caretPos.getBottom() + 1.0f), selecting);
}
...
bool TextEditor::pageUp(bool selecting) {
  ...
  const auto caretPos = (getCaretRectangle() - getTextOffset()).toFloat();
  return moveCaretWithTransaction(
      indexAtPosition(caretPos.getX(),
                      caretPos.getY() - (float)viewport->getViewHeight()),
      selecting);
}
...
bool TextEditor::pageDown(bool selecting) {
  ...
  return moveCaretWithTransaction(
      indexAtPosition(caretPos.getX(),
                      caretPos.getBottom() + (float)viewport->getViewHeight()),
      selecting);
}
...
bool TextEditor::moveCaretToStartOfLine(bool selecting) {
  const auto caretPos = (getCaretRectangle() - getTextOffset()).toFloat();
  return moveCaretWithTransaction(indexAtPosition(0.0f, caretPos.getY()),
                                  selecting);
}
...
bool TextEditor::moveCaretToEndOfLine(bool selecting) {
  const auto caretPos = (getCaretRectangle() - getTextOffset()).toFloat();
  return moveCaretWithTransaction(
      indexAtPosition((float)textHolder->getWidth(), caretPos.getY()),
      selecting);
}
...
//==============================================================================
void TextEditor::setEscapeAndReturnKeysConsumed(
    bool shouldBeConsumed) noexcept {
...
bool TextEditor::keyStateChanged(const bool isKeyDown) {
  ...
  if (KeyPress(KeyPress::F4Key, ModifierKeys::altModifier, 0).isCurrentlyDown())
    return false; // We need to explicitly allow alt-F4 to pass through on
                  // Windows
#endif
...
void TextEditor::handleCommandMessage(const int commandId) {
  ...
  case TextEditorDefs::textChangeMessageId:
    listeners.callChecked(
        checker, [this](Listener &l) { l.textEditorTextChanged(*this); });
  ...
  case TextEditorDefs::returnKeyMessageId:
    listeners.callChecked(
        checker, [this](Listener &l) { l.textEditorReturnKeyPressed(*this); });
  ...
  case TextEditorDefs::escapeKeyMessageId:
    listeners.callChecked(
        checker, [this](Listener &l) { l.textEditorEscapeKeyPressed(*this); });
  ...
  case TextEditorDefs::focusLossMessageId:
    updateValueFromText();
    listeners.callChecked(
        checker, [this](Listener &l) { l.textEditorFocusLost(*this); });
...
void TextEditor::setTemporaryUnderlining(
    const Array<Range<int>> &newUnderlinedSections) {
...
void TextEditor::insert(const String &text, int insertIndex, const Font &font,
                        Colour colour, UndoManager *um,
                        int caretPositionToMoveTo) {
  if (text.isNotEmpty()) {
    if (um != nullptr) {
      if (um->getNumActionsInCurrentTransaction() >
          TextEditorDefs::maxActionsPerTransaction)
        newTransaction();

      um->perform(new InsertAction(*this, text, insertIndex, font, colour,
                                   caretPosition, caretPositionToMoveTo));
    } else {
      repaintText({insertIndex,
                   getTotalNumChars()}); // must do this before and after
                                         // changing the data, in case a line
                                         // gets moved due to word wrap
      ...
      for (int i = 0; i < sections.size(); ++i) {
        ...
        if (insertIndex == index) {
          sections.insert(
              i, new UniformTextSection(text, font, colour, passwordCharacter));
          break;
        }
...
void TextEditor::reinsert(
    int insertIndex, const OwnedArray<UniformTextSection> &sectionsToInsert) {
  int index = 0;
  int nextIndex = 0;
  ...
  for (int i = 0; i < sections.size(); ++i) {
    ...
    if (insertIndex == index) {
      for (int j = sectionsToInsert.size(); --j >= 0;)
        sections.insert(
            i, new UniformTextSection(*sectionsToInsert.getUnchecked(j)));
    ...
    if (insertIndex > index && insertIndex < nextIndex) {
      ...
      for (int j = sectionsToInsert.size(); --j >= 0;)
        sections.insert(
            i + 1, new UniformTextSection(*sectionsToInsert.getUnchecked(j)));
...
void TextEditor::remove(Range<int> range, UndoManager *const um,
  ...
  if (!range.isEmpty()) {
    ...
    if (um != nullptr) {
      ...
      for (int i = 0; i < sections.size(); ++i) {
        ...
        if (remainingRange.getStart() <= index &&
            remainingRange.getEnd() >= nextIndex) {
          sections.remove(i);
...
bool TextEditor::isEmpty() const { return getTotalNumChars() == 0; }

void TextEditor::getCharPosition(int index, Point<float> &anchor,
...
public:
  explicit EditorAccessibilityHandler(TextEditor &textEditorToWrap)
      : AccessibilityHandler(
            textEditorToWrap,
            textEditorToWrap.isReadOnly() ? AccessibilityRole::staticText
...
private:
  ...
  public:
    explicit TextEditorTextInterface(TextEditor &editor) : textEditor(editor) {}
    ...
    String getText(Range<int> r) const override {
      if (isDisplayingProtectedText())
        return String::repeatedString(
            String::charToString(textEditor.getPasswordCharacter()),
    ...
    RectangleList<int> getTextBounds(Range<int> textRange) const override {
      ...
      std::for_each(localRects.begin(), localRects.end(),
                    [&](const Rectangle<int> &r) {
                      globalRects.add(textEditor.localAreaToGlobal(r));
                    });
    ...
    int getOffsetAtPoint(Point<int> point) const override {
      return textEditor.getTextIndexAt(
          textEditor.getLocalPoint(nullptr, point));
    }
