...
struct AudioVisualizerComponent::ChannelInfo
{
    ChannelInfo (AudioVisualizerComponent& o, int bufferSize)
    ...
    AudioVisualizerComponent& owner;
...
//==============================================================================
AudioVisualizerComponent::AudioVisualizerComponent (const int initialNumChannels)
  : numSamples (1024),
...
AudioVisualizerComponent::~AudioVisualizerComponent()
{
}

void AudioVisualizerComponent::setNumChannels (const int numChannels)
{
    channels.clear();
...
void AudioVisualizerComponent::setBufferSize (int newNumSamples)
{
...
void AudioVisualizerComponent::clear()
{
    for (int i = 0; i < channels.size(); ++i)
        channels.getUnchecked(i)->clear();
}

void AudioVisualizerComponent::pushBuffer (const float** d, int numChannels, int num)
{
...
void AudioVisualizerComponent::pushBuffer (const AudioBuffer<float>& buffer)
{
...
void AudioVisualizerComponent::pushBuffer (const AudioSourceChannelInfo& buffer)
{
...
void AudioVisualizerComponent::pushSample (const float* d, int numChannels)
{
    numChannels = jmin (numChannels, channels.size());
...
void AudioVisualizerComponent::setSamplesPerBlock (int newSamplesPerPixel) noexcept
{
    inputSamplesPerBlock = newSamplesPerPixel;
}

void AudioVisualizerComponent::setRepaintRate (int frequencyInHz)
{
    startTimerHz (frequencyInHz);
}

void AudioVisualizerComponent::timerCallback()
{
    repaint();
}

void AudioVisualizerComponent::setColors (Color bk, Color fg) noexcept
{
    backgroundColor = bk;
    waveformColor = fg;
    repaint();
...
void AudioVisualizerComponent::paint (Graphics& g)
{
...
void AudioVisualizerComponent::getChannelAsPath (Path& path, const Range<float>* levels, int numLevels, int nextSample)
{
...
void AudioVisualizerComponent::paintChannel (Graphics& g, Rectangle<float> area,
