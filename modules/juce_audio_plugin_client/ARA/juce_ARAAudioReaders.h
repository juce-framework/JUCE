...
public:
    /** Use a std::vector<ARAPlaybackRegion*> to construct a playback region reader. 
        ...
        @param playbackSampleRate The sample rate at which \p renderer should render playback regions (the first region's audio source sample rate will be used if left 0.0)
    ...
    */
    ARAPlaybackRegionReader (ARAPlaybackRenderer* renderer, std::vector<ARAPlaybackRegion*> const& playbackRegions, bool nonRealtime, double playbackSampleRate = 0.0, int channelCount = 0, bool use64BitSamples = false);
...
public:
    /** Use an ARARegionSequence to construct a region sequence reader. 
        ...
        @param nonRealtime Whether or not the samples need to be read in real time (the \p renderer will be configured appropriately)
        @param playbackSampleRate The sample rate at which \p renderer should render playback regions (will use the first region's audio source sample rate if left 0.0)
    ...
    */
    ARARegionSequenceReader (ARAPlaybackRenderer* renderer, ARARegionSequence* regionSequence, bool nonRealtime, double playbackSampleRate = 0.0, int channelCount = 0, bool use64BitSamples = false);
